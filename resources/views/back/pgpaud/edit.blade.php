			...
			<div class="card full-height">
				<div class="card-header">
					<div class="card-head-row">
						<div class="card-title">Edit <i>{{ $pgpaud->nama }}</i></div>
                        <a href="{{ route('pgpaud.index') }}" class="btn btn-warning btn-sm ml-auto"><i class="fa fa-arrow-left"></i> Back</a>
					</div>
				</div>
				<div class="card-body">
					<form method="POST" action="{{ route('pgpaud.update', $pgpaud->id) }}" enctype="multipart/form-data">
						@csrf
                        @method('PUT')
						<div class="form-group">
							<label for="nidn">NIDN/NITK</label>
							<input type="text" name="nidn" class="form-control" id="text" value="{{ $pgpaud->nidn }}">
						</div>
						<div class="form-group">
							<label for="nama">Nama</label>
							<input type="text" name="nama" class="form-control" id="text" value="{{ $pgpaud->nama }}">
						</div>
                        <div class="form-group">
							<label for="jabatan">Jabatan</label>
							<select type="text" name="jabatan_id" class="form-control" id="text">
                                @foreach ($jabatan as $row)
                                    @if ($row->id == $pgpaud->jabatan_id)                                        
                        ...
                        <div class="form-group">
                            <label for="foto">Gambar pgpaud</label>
                            ...
                            <img src="{{ asset('uploads/'.$pgpaud->foto) }}" width="150" class="img-fluid">
                        </div>
						<div class="form-group">
